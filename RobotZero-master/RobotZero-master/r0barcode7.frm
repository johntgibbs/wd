VERSION 5.00
Begin VB.Form Form7 
   Caption         =   "Blue Bell Pallets - Robot Zero"
   ClientHeight    =   8535
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   11610
...
Private Sub barcode_scanned(bc As String)
    ...
    Combo2.Clear
    Combo2.AddItem " "
    For i = 0 To DateDiff("d", cd, td)                              'jv041918
...
Private Sub refresh_plate(pno As Long)
    ...
    'db.Open Form1.bbsr
    If pno = 0 Then
        s = "select sequence_id from sequences where seq = 'R0Barcode'"
        Set ds = Wdb.Execute(s)
        If ds.BOF = False Then
            ds.MoveFirst
        ...
        ds.Close
    Else
        s = "update sequences set sequence_id = " & pno & " where seq = 'R0Barcode'"
        Wdb.Execute (s)
