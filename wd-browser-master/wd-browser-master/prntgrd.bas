...
Sub printflexgrid(pd As Control, gn As Control, rt As String, rh As String, rf As String)
    ...
    For i = 1 To gn.Rows - 1
        If header Then
            If TypeOf pd Is Printer And i > 1 Then pd.NewPage
...
Sub htmlcolorgrid(fn As Form, hf As String, gn As Control, rt As String, rh As String, rf As String, pc As String, hc As String, dc As String)
    Dim i As Integer, k As Integer, tw As Long, j As Integer, msg As String
    Dim cc As Long
    If Len(pc) = 0 Then pc = "linen"
    If Len(hc) = 0 Then hc = "lightgrey"
    If Len(dc) = 0 Then dc = "white"
    If Len(hf) = 0 Then hf = localAppDataPath & "\htmlgrid.htm"
    'msg = MsgBox(hf, vbOKOnly)
    Open hf For Output As #1
    Print #1, "<html>"
    Print #1, "<head><title>"; rt; "</title></head>"
    Print #1, "<body bgcolor="; pc; ">"
...
Sub lsjgrid(d As String, u As String, pwd As String, q As String, wf As String, gn As Control, lf As String, gcs As Integer)
    Dim rfile As String
    rfile = lf & "\rrequest.txt"
    If Len(Dir(wf)) > 0 Then Kill wf
    Do
        DoEvents
    Loop Until Len(Dir(rfile)) = 0
    
    Open rfile For Output As #1
    Write #1, d, u, pwd, q, wf, ".."
    Close #1
    Do
        DoEvents
    Loop Until Len(Dir(wf)) > 0
    gn.Clear: gn.Rows = 1: gn.Cols = gcs
    Open wf For Input As #1
    Do Until EOF(1)
        Line Input #1, s
        gn.AddItem s
    Loop
