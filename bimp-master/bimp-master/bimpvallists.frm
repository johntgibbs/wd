...
Begin VB.Form bimpvallists 
   ...
   End
End
Attribute VB_Name = "bimpvallists"
...
Private Sub refresh_grid()
    ...
    s = s & " order by listreturn, listdisplay"
...
Private Sub refresh_list()
    ...
    'Set db = CreateObject("ADODB.Connection")
    'db.Open Form1.BBSR
    'If Form1.userid = "jvierus" Then
    '    s = "select distinct listname from valuelists order by listname"
    'Else
    '    s = "select distinct listname from valuelists where listname not in ('wmsmenu', 'wmsuser')"
    '    s = s & " order by listname"
    'End If
    'Set ds = db.Execute(s)
    'If ds.BOF = False Then
    '    ds.MoveFirst
    ...
    'ds.Close: db.Close
    'MsgBox vkey.Caption
    If vkey.Caption = "vkey" Then                       'jv010417
        Combo1.AddItem "bimpproddates"
        Combo1.AddItem "wdserverstatus"                 'jv010417
    Else
        Combo1.AddItem vkey.Caption                     'jv010417
    End If
    Combo1.ListIndex = 0
    Exit Sub
vberror:
...
Private Sub Command1_Click()
    ...
    If ds.BOF = False Then
        ...
        wdb.Execute s
        'ds.Edit
        'ds!listreturn = Grid1.TextMatrix(Grid1.Row, 1)
        'ds!listdisplay = Grid1.TextMatrix(Grid1.Row, 2)
        'ds.Update
    End If
    ds.Close ': db.Close
...
Private Sub Command2_Click()
    Dim db As ADODB.Connection, ds As ADODB.Recordset, s As String, nc As String, i As Integer, zid As Long
    Dim eno As Long, edesc As String
    On Error GoTo vberror
    'zid = wd_seq("Valuelists", Form1.BBSR)
    zid = wd_seq("Valuelists")
    'Set db = CreateObject("ADODB.Connection")
    'db.Open Form1.BBSR
    s = "Insert into valuelists (id, listname, listreturn, listdisplay) values (" & zid & ", '" & Combo1 & "', ' ', ' ')"
    'MsgBox s
    wdb.Execute s
    's = "select * from valuelists where id = 0"
    'Set ds = db.Execute(s)
    'ds.AddNew
    'ds!listname = Combo1
    'nc = ds!id
...
Private Sub vkey_Change()
    If vkey <> "vkey" Then refresh_list
End Sub
