...
Begin VB.Form shipusermenu 
   ...
   End
End
Attribute VB_Name = "shipusermenu"
...
Private Sub refresh_lists()
    ...
    s = "select listreturn, listdisplay from valuelists where listname = 'wdshipuser' order by listdisplay"
    ...
    s = "select listreturn, listdisplay from valuelists where listname = 'shipmenu' order by listdisplay"
    Set ds = Sdb.Execute(s)
    If ds.BOF = False Then
        ds.MoveFirst
        Do Until ds.EOF
...
Private Sub addmenu_Click()
    ...
    If ds.BOF = False Then
        ...
        Grid1.TextMatrix(i, 0) = ds!id
    Else
        zid = wd_seq("Usermenus", Form1.shipdb)
...
Private Sub Command2_Click()
    Dim sqlx As String, v As String
    If Form1.plantno <> "50" Then
        MsgBox "This command is only used for Plant 50.", vbOKOnly + vbInformation, "Switch to Plant 50...."
        Exit Sub
    End If
    v = Form1.Label13
    sqlx = "Update valuelists set listreturn = '" & Format(DateAdd("d", 1, Now), "yyMMdd") & "'"
    sqlx = sqlx & ", listdisplay = '" & v & "' Where listname = 'latestversion'"
    'MsgBox sqlx
    Sdb.Execute sqlx
End Sub
